...
Matrix::Matrix(unsigned long width, unsigned long height) : width(width), height(height) {
    arr = vector< vector<NodeMatrix*> > (width);
    for (int i = 0; i < width ; ++i) {
        arr[i] = vector<NodeMatrix*> (height);
        for (int j = 0; j < height; ++j) {
            arr[i][j] = new NodeMatrix(i, j);
